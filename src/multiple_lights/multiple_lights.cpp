...
void opengl_uniform(GLuint shader, scene_environment_with_multiple_lights const& environment)
{
	...
	opengl_uniform(shader, scene_environment_camera_head(environment));
...
void compute_light_position(std::vector<vec3> pos, scene_environment_with_multiple_lights& environment) {

    // Adapt the position and color of the spotlight procedurally as a function of t
    environment.spotlight_color.resize(pos.size());
    environment.spotlight_position.resize(pos.size());

    for (int i = 0;i<pos.size();i++) {
        environment.spotlight_color[i] = { 0, 0.0f, 1.0f };
        environment.spotlight_position[i] = pos[i];
    }
}

void light_shape_drawable::initialize(GLuint shader_multiple_lights)
{
	float const radius_sphere = 0.01f; // Adapt the radius of the spotlight as you wish
